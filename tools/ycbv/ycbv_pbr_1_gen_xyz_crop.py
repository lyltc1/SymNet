...
xyz_root = osp.normpath(osp.join(PROJ_ROOT, "datasets/xyz_crop/ycbv/train_pbr"))
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        print(
                            f"not visible, split {split} scene {scene_id}, im {int_im_id} obj {obj_id}"
                        )
                        print(f"{save_path}")
                        xyz_info = {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen ycbv train_real xyz")
