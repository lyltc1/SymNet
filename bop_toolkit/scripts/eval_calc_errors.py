...
for result_filename in p['result_filenames']:
  ...
  # Load object models.
  models = {}
  if p['error_type'] in ['ad', 'add', 'adi', 'mssd', 'mspd', 'proj', 'AUCadd', 'AUCadi']:
          ...
          for gt_id, gt in enumerate(scene_gt[im_id]):
            ...
            if p['error_type'] == 'vsd':
              ...
              if sphere_projections_overlap:
                ...
                e = [1.0]
            elif p['error_type'] in ['AUCadd', 'AUCadi']:
              if p['error_type'] in ['AUCadd']:
                e = [pose_error.add(
                  R_e, t_e, R_g, t_g, models[obj_id]["pts"],)  / 10]  # mm to cm
              elif p['error_type'] in ['AUCadi']:
                e = [pose_error.adi(
                  R_e, t_e, R_g, t_g, models[obj_id]["pts"],)  / 10]  # mm to cm
              else:
                misc("some error here")
