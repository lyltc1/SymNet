...
folder_path = "/home/Symnet/output/tless_pbr_only/zebrapose"

found = []

for root, dirs, files in os.walk(folder_path):
    for file in files:
        if file.startswith("zebrapose_tless-test_obj") and file.endswith(".csv"):
            found.append(os.path.join(root, file))

assert len(found) == 30

combined_csv = pd.concat([pd.read_csv(f) for f in found])
output_fn = os.path.join(folder_path, "Zebrapose_tless-test.csv")
