...
dir_suffix = "evalfcos"
out_csv_name = "SymNetPbrFcos_tless-test.csv"
...
if GENERATE_RESULT:
    ...
    for eval_folder in eval_folders:
        command = f"python core/symn/run_evaluate.py --eval_folder {os.path.join(main_folder_name, eval_folder)} --detection zebrapose_detections/tless/tless_bop_pbr_only.json --dir_suffix evalfcos"
...
if EVAL_BOP_RESULT:
    # find all the file "SymNet_tless-test.csv" whose parent folder end with suffix
    eval_files = []
    ...
    find_files(main_folder_name, "SymNet_tless-test.csv", dir_suffix)

    eval_files.sort()
    assert len(eval_files) == 30
    # Print full paths
    combined_csv = pd.concat([pd.read_csv(f) for f in eval_files])
    output_fn = os.path.join(main_folder_name, out_csv_name)
