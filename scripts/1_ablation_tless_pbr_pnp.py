...
if GENERATE_RESULT:
    ...
    for eval_folder in eval_folders:
        command = f"python core/symn/ablation_run_evaluate_pnp.py --eval_folder {os.path.join(main_folder_name, eval_folder)} --detection gdrnppdet-pbr/gdrnppdet-pbr_tless-test_bed88a8e-1e0e-405b-8c62-8e7b83cf8934.json"
...
if EVAL_BOP_RESULT:
    # find all the file "SymNet_tless-test.csv" whose parent folder end with suffix 'ablation_pnp'
    eval_files = []
    ...
    find_files(main_folder_name, "SymNet_tless-test.csv", "ablation_pnp")

    eval_files.sort()
    assert len(eval_files) == 30
    # Print full paths
    combined_csv = pd.concat([pd.read_csv(f) for f in eval_files])
    output_fn = os.path.join(main_folder_name, "SymNetPbrPnP_tless-test.csv")
