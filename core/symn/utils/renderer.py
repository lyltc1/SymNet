...
class ObjCoordRenderer:
    def __init__(self, objs, obj_ids, w: int, h: int = None, device_idx=0):
        self.objs = objs
        if h is None:
            h = w
        self.h, self.w = h, w
    ...
    def render(self, obj_id, K, R, t, clear=True, read=True, read_depth=False):
        obj = self.objs[obj_id]
        mv = np.concatenate((
        ...
        if clear:
            self.ctx.clear()
        self.ctx.enable(moderngl.DEPTH_TEST)
        self.vaos[obj_id].render(mode=moderngl.TRIANGLES)
        # self.vaos[obj_idx].render(mode=moderngl.POINTS)
        if read_depth:
