...
def visulize_rgb(rgb: Union[np.ndarray, torch.Tensor]):
...
def visulize_mask(mask: Union[np.ndarray, torch.Tensor]):
...
def visualize_code(code: Union[np.ndarray, torch.Tensor]):
...
def visualize_pose(obj_id, renderer, K, R, t, rgb):
...
def visualize_v2(inputs, out_dir, out_dict=None, renderer=None, sub_file=None):
    ...
    while os.path.exists(save_path):
        i_id += 1
        save_path = join(out_dir, str(scene_id)+'_'+str(img_id)+'_'+str(i_id)+'result.txt')
    rgb_crop = visulize_rgb(inputs["rgb_crop"][0])
    # ----visualize gt mask visib
    gt_mask_amodal_crop = visulize_mask(inputs["mask_crop"][0])
    gt_mask_visib_crop = visulize_mask(inputs["mask_visib_crop"][0])
    # ----visualize gt code
    gt_code_crop = visualize_code(inputs["code_crop"][0])
    ...
    if renderer is not None:
        ...
        pose_img = visualize_pose(obj_id, renderer, K, gt_R, gt_t, rgb_crop)
        concat_img_channel3 = np.column_stack([rgb_crop, pose_img])
    if out_dict is not None:
        visib_mask_prob = visulize_mask(out_dict["visib_mask_prob"][0, 0])
        amodal_mask_prob = visulize_mask(out_dict["amodal_mask_prob"][0, 0])
        binary_code_prob = visualize_code(out_dict["binary_code_prob"][0])
        ...
        if renderer is not None:
            ...
            pose_eval_img = visualize_pose(obj_id, renderer, K, eval_R, eval_t, rgb_crop)
