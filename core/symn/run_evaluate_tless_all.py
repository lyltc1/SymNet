...
def main():
    ...
    # adapt for run tless all
    cfg.DATASETS.TEST_KEYFRAME="tless_test_targets_all.json"
    cfg.DATASETS.TEST_DETECTION_TYPE="type2"
    cfg.DATASETS.TEST_DETECTION_PATH='gdrnpp_yolox_detections/tless/test/test_bboxes/yolox_real_pbr_all_results.json'
    if args.pbr:
        cfg.DATASETS.TEST_DETECTION_PATH='gdrnpp_yolox_detections/tless/test/test_bboxes/yolox_pbr_all_results.json'
        print("pbr only")
    ...
    for ck in args.ckpt:
        cfg.OUTPUT_DIR = os.path.join(cfg.OUTPUT_ROOT, "result_all", os.path.splitext(ck)[0])
