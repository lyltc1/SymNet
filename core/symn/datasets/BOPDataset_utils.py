...
def get_aux(
    cfg, gt, aug_bg=False, aug_rgb=False, aug_occ=False, detection=False, debug=False
):
    auxs = [
        RgbLoader(),
    ]
    if gt is True:
        if detection:
            crop_aux = RandomRotatedMaskCrop(
                cfg.DATASETS.RES_CROP,
                use_bbox_est=True,
                offset_scale=0.0,
        ...
        if aug_bg is True:
            auxs.extend(
                [
                    ReplaceBgAux(cfg.DATASETS.BG_AUG_PROB, cfg.DATASETS.BG_AUG_TYPE),
                ]
            )
        if aug_rgb is True:
            auxs.extend(
        ...
        auxs.extend(
            [
                crop_aux.apply_aux_d2,
                GT2CodeAux(),
                PosePresentationAux(
                    cfg.DATASETS.RES_CROP,
                    R_type=cfg.MODEL.PNP_NET.R_type,
                    t_type=cfg.MODEL.PNP_NET.t_type,
                ),
...
def build_BOP_train_dataset(cfg, dataset_type, debug=False):
    ...
    for name in dataset_type:
        if name in ["train_real", "train_primesense", "train"]:
            # we use both ReplaceBgAux and AugRgbAux in real dataset
            auxs_real = get_aux(
                cfg, gt=True, aug_occ=True, aug_bg=True, aug_rgb=True, debug=debug
            )
            real_dataset = BopTrainDataset(
                meta_info,
                name,
                obj_ids=obj_ids,
                code_type=cfg.DATASETS.get("CODE_TYPE", "SymCode"),
                auxs=auxs_real,
            )
            ...
            auxs_pbr = get_aux(
                cfg, gt=True, aug_occ=True, aug_bg=False, aug_rgb=True, debug=debug
            )
            pbr_dataset = BopTrainDataset(
                meta_info,
                name,
                obj_ids=obj_ids,
                code_type=cfg.DATASETS.get("CODE_TYPE", "SymCode"),
    ...
    for folder_name in dataset_type:
        if folder_name in ["test", "test_primesense"]:
            ...
            if use_detection:
                ...
                get_det_fun_name = (
                    "get_detection_results_" + cfg.DATASETS.TEST_DETECTION_TYPE
                )
                get_det_module = importlib.import_module(
                    ".get_detection_results", "datasets"
                )
                get_det_fun = getattr(get_det_module, get_det_fun_name)
                detections = get_det_fun(
                    join(meta_info.detections_folder, cfg.DATASETS.TEST_DETECTION_PATH)
                )

            # ---- get keyframe ----
            keyframe = cfg.DATASETS.TEST_KEYFRAME
            # ---- build dataset ----
            test_dataset = BopTestDataset(
                meta_info,
                folder_name,
                obj_ids=obj_ids,
                code_type=cfg.DATASETS.get("CODE_TYPE", "SymCode"),
...
def batch_data_train(data):
    ...
    for key in [
        ...
        batch[key] = torch.stack(
            [torch.tensor(d[key], dtype=torch.float32) for d in data], dim=0
        )
    for key in ["AABB_crop"]:
        batch[key] = torch.stack(
            [torch.tensor(d[key], dtype=torch.long) for d in data], dim=0
        )
