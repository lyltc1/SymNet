...
class BopTrainDataset(torch.utils.data.Dataset):
    def __init__(
            ...
            auxs: Sequence['BopDatasetAux'] = tuple(), show_progressbar=True,
    ):
        self.meta_info = meta_info
        bop_dataset_folder = meta_info.bop_dataset_folder
        self.data_folder = join(bop_dataset_folder, folder_name)
        self.GT_folder = join(meta_info.binary_code_folder, folder_name + '_GT')
        ...
        if scene_ids is None:
            scene_ids = sorted([int(p.split('/')[-1]) for p in glob.glob(self.data_folder + '/*')])
        ...
        if keyframe is not None and detections is not None:
            ...
            for scene_id, im_id, obj_id, _ in targets_list:
                ...
                if current_scene_id != scene_id:
                    ...
                    scene_folder = join(self.data_folder, f'{scene_id:06d}')
                    scene_camera = load_json(join(scene_folder, 'scene_camera.json'), keys_to_int=True)
                    if exists(join(scene_folder, 'scene_gt_info.json')):
                        has_gt = True
                        scene_gt_info = load_json(join(scene_folder, 'scene_gt_info.json'), keys_to_int=True)
                        scene_gt = load_json(join(scene_folder, 'scene_gt.json'), keys_to_int=True)
                    else:
                        has_gt = False
                ...
                if not has_gt:
                    for found_detection in found_detections:
                        ...
                        self.instances.append(dict(
                            scene_id=scene_id, img_id=im_id, K=K, obj_id=obj_id, bbox_est=bbox_est,
                            obj_idx=obj_idxs[obj_id], det_score=det_score, det_time=det_time,
                        ))
                    ...
                    for found_detection in found_detections:
                        ...
                        for i, (gt_info_this_img_i, gt_this_img_i) in enumerate(
                            ...
                            if gt_this_img_i['obj_id'] == obj_id:
                                ...
                                if iou > iou_between_detection_and_gt:
                                    ...
                                    gt_index = i
                        cam_R_obj = np.array(gt_for_found_detection['cam_R_m2c']).reshape(3, 3)
                        cam_t_obj = np.array(gt_for_found_detection['cam_t_m2c']).reshape(3, 1)
                        ...
                        det_time = found_detection['det_time']
                        self.instances.append(dict(
                            scene_id=scene_id, img_id=im_id, K=K, obj_id=obj_id, pose_idx=gt_index,
                            bbox_visib=bbox_visib, bbox_obj=bbox_obj, cam_R_obj=cam_R_obj, cam_t_obj=cam_t_obj,
                            obj_idx=obj_idxs[obj_id], det_score=det_score, det_time=det_time, bbox_est=bbox_est,
            ...
            for scene_id, im_id, obj_id, _ in targets_list:
                ...
                scene_gt_this_img = scene_gt[im_id]
                scene_gt_info_this_img = scene_gt_info[im_id]
                for i, (gt_info_this_img_i, gt_this_img_i) in enumerate(zip(scene_gt_info_this_img, scene_gt_this_img)):
                    if gt_this_img_i['obj_id'] != obj_id:
                        continue
                    ...
                    bbox_obj = [x1, y1, x1 + w, y1 + h]
                    self.instances.append(dict(
                        scene_id=scene_id, img_id=im_id, K=K, obj_id=obj_id, pose_idx=i,
                        bbox_visib=bbox_visib, bbox_obj=bbox_obj, cam_R_obj=cam_R_obj, cam_t_obj=cam_t_obj,
                        obj_idx=obj_idxs[obj_id],
                    ))
        else:
