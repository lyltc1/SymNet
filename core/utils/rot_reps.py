...
def ortho6d_to_mat_with_axis_batch(poses, axis):
    # poses bx6
    # poses
    axis_1_raw = poses[:, 0:3]  # bx3
    axis_2_raw = poses[:, 3:6]  # bx3

    axis_1 = normalize_vector(axis_1_raw)  # bx3
    axis_3 = cross_product(axis_1, axis_2_raw)  # bx3
    axis_3 = normalize_vector(axis_3)  # bx3
    axis_2 = cross_product(axis_3, axis_1)  # bx3

    axis_1 = axis_1.view(-1, 3, 1)
    axis_2 = axis_2.view(-1, 3, 1)
    axis_3 = axis_3.view(-1, 3, 1)
    if axis == 0:
        matrix = torch.cat((axis_1, axis_2, axis_3), 2)  # bx3x3
    elif axis == 1:
        matrix = torch.cat((axis_3, axis_1, axis_2), 2)  # bx3x3
    elif axis == 2:
        matrix = torch.cat((axis_2, axis_3, axis_1), 2)  # bx3x3
    return matrix
